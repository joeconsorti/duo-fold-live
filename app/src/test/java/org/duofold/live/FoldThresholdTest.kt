...
class FoldThresholdTest {
 @Test fun closedDefaultIsTwoButOneRemainsSelectable(){assertEquals(2f,FoldThreshold.sanitizeClosed(Float.NaN),0f);assertEquals(1f,FoldThreshold.sanitizeClosed(1f),0f);assertEquals(0f,FoldThreshold.effectiveAngle(2f,2f),0f);assertEquals(2f,FoldThreshold.effectiveAngle(2f,1f),0f)}
