...
public class FoldRotationPolicyTest {
 ...
 @Test public void handoffGapDoesNotUnlock(){FoldRotationPolicy p=new FoldRotationPolicy();assertTrue(p.update(0,true,true,80,172));assertTrue(p.update(100,true,false,80,172));assertTrue(p.update(900,true,true,150,172));assertTrue(p.update(1000,true,true,179,172));assertFalse(p.update(1450,true,true,179,172));}
 @Test public void animationThresholdCannotReleaseRotationEarly(){FoldRotationPolicy p=new FoldRotationPolicy();assertFalse(p.update(0,true,true,179,165));assertTrue(p.update(10,true,true,177,165));assertTrue(p.update(600,true,true,170,165));assertTrue(p.update(1000,true,true,178,165));assertFalse(p.update(1450,true,true,179,165));}
