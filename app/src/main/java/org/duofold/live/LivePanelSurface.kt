...
internal class LivePanelSurface(context:Context,private val result:(Boolean,String)->Unit):SurfaceView(context),SurfaceHolder.Callback {
 ...
 private val retry=object:Runnable{override fun run(){
  ...
  if(ready && !pending && !attached){
   ...
   id=LiveAngles.attachMirror(surfaceControl,mirrorWidth,mirrorHeight){ok,note->
    if(gen==generation){pending=false;attached=ok;revision=requestedRevision;result(ok,note)}
   }
  }
 ...
 override fun surfaceChanged(holder:SurfaceHolder,format:Int,width:Int,height:Int){
  ...
  mirrorWidth=width;mirrorHeight=height;active=true;post(retry)
 }
 override fun surfaceDestroyed(holder:SurfaceHolder){close()}
 fun close(){active=false;removeCallbacks(retry);generation++;pending=false;attached=false;revision=-1;if(id!=0)LiveAngles.detachMirror(id);id=0;result(false,"Live mirror released")}
}
