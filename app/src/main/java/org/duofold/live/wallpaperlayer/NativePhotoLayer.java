...
public final class NativePhotoLayer extends DisplayAreaOrganizer {
 ...
 void attach(DisplayAreaInfo info,SurfaceControl parent)throws Exception{
  ...
  IBinder key=info.token.asBinder();Entry prior=layers.remove(key);if(prior!=null){if(prior.wake!=null)independentCount--;prior.remove();}
  ...
  Entry e=new Entry(info,parent,child);layers.put(key,e);
  try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()){
   t.setBuffer(child,buffer).setLayer(child,Integer.MAX_VALUE).setAlpha(child,1f);geometry(t,e);t.setVisibility(child,true);t.apply();
  }
 ...
 void geometry(SurfaceControl.Transaction t,Entry e){
  try{
   Object wc=e.info.configuration.getClass().getField("windowConfiguration").get(e.info.configuration);
 ...
 public void onDisplayAreaInfoChanged(DisplayAreaInfo info){if(closed)return;Entry e=layers.get(info.token.asBinder());if(e==null)return;String config=info.configuration.toString();if(config.equals(e.lastConfiguration))return;e.lastConfiguration=config;e.info=info;try(SurfaceControl.Transaction t=new SurfaceControl.Transaction()){geometry(t,e);t.apply();}catch(Throwable error){failed.accept(error);}}
