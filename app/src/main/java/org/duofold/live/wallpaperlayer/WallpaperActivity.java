...
public class WallpaperActivity extends Activity {
 ...
 button(connectionCard,"Authorize Shizuku",()->{try{if(!Shizuku.pingBinder())status.setText("Start Shizuku first.");else if(Shizuku.checkSelfPermission()==0){status.setText("Shizuku authorized. Reconnecting report observer…");reconnectForReport();}else Shizuku.requestPermission(1);}catch(Exception e){status.setText(e.toString());}});
 ...
 final Runnable poll=new Runnable(){public void run(){try{status.setText((WallpaperRestore.enabled(WallpaperActivity.this)?"Enabled":"Disabled")+" · "+WallpaperRestore.status);}catch(Exception e){status.setText("Background status unavailable. Copy the background report for details.");}h.postDelayed(this,1000);}};
