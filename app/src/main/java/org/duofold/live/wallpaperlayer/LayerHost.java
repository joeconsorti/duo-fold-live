...
public class LayerHost extends Binder {
 ...
 void refresh(){if(homePhoto!=null)homePhoto.refreshGeometry();if(nativePhoto!=null)nativePhoto.refreshDisplays();operation="Enumerate displays";DisplayManager dm=(DisplayManager)context.getSystemService(Context.DISPLAY_SERVICE);Set<Integer> keep=new HashSet<>();for(Display d:dm.getDisplays()){
  ...
  View view=new View(wc){boolean traceNextDraw=true;
   ...
   protected void onAttachedToWindow(){super.onAttachedToWindow();trace("Display "+id+" view attached");getViewTreeObserver().addOnPreDrawListener(()->{if(traceNextDraw||getTag()!=null){String reason=getTag()==null?"attach/resize/visibility":String.valueOf(getTag());if(isHardwareAccelerated())getViewTreeObserver().registerFrameCommitCallback(()->main.post(()->trace("Display "+id+" photo frame committed; "+reason)));}return true;});}
