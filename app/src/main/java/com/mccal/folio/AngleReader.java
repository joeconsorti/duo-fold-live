...
public class AngleReader extends Binder {
 public static final String DESCRIPTOR="com.consorti.wallpaperprobe.AngleReader";
 ...
 protected synchronized boolean onTransact(int code,Parcel data,Parcel reply,int flags)throws RemoteException {
  ...
  if(code==1){String action=data.readString();if(action==null||!action.matches("com\\.consorti\\.wallpaperprobe\\.READ_[0-9]+"))throw new IllegalArgumentException("Invalid action");start(action);reply.writeNoException();return true;}
  if(code==2){heartbeat=SystemClock.elapsedRealtime();boolean unlocked=data.readInt()!=0,dual=data.readInt()!=0,primaryInner=data.readInt()!=0,secondaryReady=data.readInt()!=0;int frozenSource=data.readInt();float openThreshold=data.readFloat();
