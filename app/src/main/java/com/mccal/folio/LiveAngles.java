...
public final class LiveAngles {
 ...
 public void start(){
  ...
  try{
   ...
   running=true;current=this;last=0;count=0;action="com.consorti.wallpaperprobe.READ_"+SystemClock.elapsedRealtime();
   ensureAnchors();
   thread=new HandlerThread("folio-angle-ipc");thread.start();worker=new Handler(thread.getLooper());
   args=new Shizuku.UserServiceArgs(new ComponentName(context,AngleReader.class)).daemon(false).processNameSuffix("fold_angles").version(142);
