    ...
    Surface(Modifier.fillMaxSize()){
     Column(Modifier.fillMaxSize().safeDrawingPadding().verticalScroll(rememberScrollState()).padding(20.dp),verticalArrangement=Arrangement.spacedBy(16.dp)){
      ...
      SettingsCard("Custom wallpaper","Your photo behind the icons, with Samsung’s hinge wallpaper still running underneath."){
       Button(onClick={startActivity(Intent(this@MainActivity,com.consorti.wallpaperlayer.WallpaperActivity::class.java))}){Text("Choose & manage custom wallpaper")}
      ...
      Text("Duo Fold Live 1.5.0 · Glass reference: chuspeeism/iphone-duo (MIT). Folio-derived diagnostics. Screen frames stay in memory.",style=MaterialTheme.typography.bodySmall,color=MaterialTheme.colorScheme.onSurfaceVariant)
     }
    }
   }
  }
